...
import { LEAGUE_CONVERSION_RATES } from './constants';
...
export class ArchetypeEngine {
    ...
    static compute(matches: MatchHistory[]): typeof ARCHETYPE_STATS {
        ...
        const teamStats: Record<string, { xG: number; xGA: number; games: number; cleanSheets: number; score: number }> = {};
        ...
        matches.forEach(m => {
            ...
            [hId, aId].forEach(id => {
                if (!teamStats[id]) teamStats[id] = { xG: 0, xGA: 0, games: 0, cleanSheets: 0, score: 0 };
            });
        ...
        const getTier = (startPct: number, endPct: number) => {
            ...
            let tXG = 0, tXGA = 0, tCS = 0, n = group.length;
            group.forEach(([_, s]) => {
                tXG += s.xG / s.games;
                tXGA += s.xGA / s.games;
